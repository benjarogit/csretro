#!/usr/bin/env bash
# PX3B: Offscreen-World-Probe. Sichtbarer Frame bleibt Xash (GL_RenderFrame = 0).
# Usage: ./scripts/build-client.sh && ./scripts/px3b-offscreen-probe.sh
set -euo pipefail
...
fail() { echo "PX3B_PROBE FAIL: $*" >&2; exit 1; }
...
csretro_gate_isolate_begin "px3b" || fail "isolate"
...
r_csretro_probe_seq 1
mp_auto_join_team 0
bot_quota 0
sv_lan 1
echo CSRETRO_PX3B_PROBE_CFG
EOF
...
export CSRETRO_GAMESCOPE_LOG="${RUN}/gamescope-px3b.log"
...
wait_log 'offscreen world map=maps/de_aztec' 30 || {
	kill -TERM "${XASH_PID}" 2>/dev/null || true
	csretro_headless_x11_stop
...
if [[ -f "${RUN}/cstrike/csretro_offscreen.ppm" ]]; then
	echo "PX3B_PROBE ppm=${RUN}/cstrike/csretro_offscreen.ppm"
fi
exit 0
