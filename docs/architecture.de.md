...
│   └── PrimeXT-derived Technik (`client/render/`; PX3B World/BSP, PX3C Entity/Sprite, PX4A Non-Player Studio, PX4B.1 FOLLOW, PX4B.2 Remote-Player Variante B, PX4B.3 Local Eligibility + Player Shadows, PX4C.1 Viewmodel BODY VERIFIED, PX4C.2 Event-Ownership VERIFIED, PX5 Vis-Seam + PX5.1 Sky/Trans hinter return 0, #7 Brush Special A/B/C/D/E complete + Engine-EFX + Client-Triangles draw-only)
...
| `client/render/` | CS-Retro-Renderer (Lifecycle, Offscreen-GL, World/BSP, Brush-Entities, Sprite, Non-Player + Remote-Player Studio). Eine `client_amd64.so`. |
...
PX1-Research und Port-Matrix: [docs/research/px1-primext.md](research/px1-primext.md). PX2 (`17bd79f`) ist die `HUD_GetRenderInterface`-Brücke. PX3A: Strategie C. PX3B (`bbe418d`, visuell zertifiziert): World/Offscreen. PX3C (visuell zertifiziert, #6 geschlossen): Entity-Spiegel + Sprite-Offscreen. PX4A (visuell zertifiziert, #8 geschlossen): Non-Player Studio offscreen über GSMR `STUDIO_RENDER`. PX4B.1: Non-Player FOLLOW-Pfad vorhanden, auf Stock-CS-Maps nicht reproduzierbar. PX4B.2: Remote-Player Variante B isoliert (gemeinsames GSMR, lokale `player_info_t`, Live mutate=0). PX4B.3: Local Xash-Eligibility + explizite Player-Shadows VERIFIED, Issue #9 geschlossen. Sichtbares GSMR: `m_bLocal` false, `SetupClientAnimation` inaktiv. #7 Brush Special A/B/C/D/E complete, Issue geschlossen. Engine-EFX draw-only hinter `GL_RenderFrame` 0; Client-Triangles VERIFIED draw-only (Spectator Map-Overview Cert); Sprite Completion (ANGLED implemented / runtime NOT REPRODUCIBLE, lerp VERIFIED, Xash sprite lighting VERIFIED). PX5 Vis-Seam hinter return 0: Tail-API `PrepareCurrentFrameVis` plus `R_SetupGL(false)` für `farClip`, Client-PVS-Puffer, World-Surface-Maske, efrag-Inventar, Sky-Pixelproof, `GetEntityRenderInfoReadOnly` (`REF_API_VERSION` 24). Issue [#11](https://github.com/benjarogit/csretro/issues/11) CLOSED. Viewmodel [#10](https://github.com/benjarogit/csretro/issues/10) CLOSED (PX4C.1 Body + PX4C.2 Event-Ownership VERIFIED). Sichtbarer Takeover (`return 1`) erst nach neuer Freigabe. Variante C ist nicht mehr festgeschrieben.
