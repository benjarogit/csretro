...
int _V_UnicodeToUCS2( const wchar_t *pUnicode, int cubSrcInBytes, char *pUCS2, int cubDestSizeInBytes )
{
	...
	// counts.
	int cchResult = std::min( (size_t)cubSrcInBytes/sizeof(wchar_t), cubDestSizeInBytes/sizeof(wchar_t) );
...
static inline struct tm *Plat_localtime(const time_t *timep, struct tm *result)
{
#ifdef _WIN32
