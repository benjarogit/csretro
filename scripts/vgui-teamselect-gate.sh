#!/usr/bin/env bash
# Team-Select Gate: In-Game Team-Wahl als echte VGUI2-Controls.
# Startet de_dust (kein Auto-Join), prüft Teammenu.res, Slots, Localization,
# Kartenbeschreibung, ESC, Taste 2 → jointeam 2, dann quit.
# Usage: ./scripts/build-menu.sh && ./scripts/build-client.sh && ./scripts/vgui-teamselect-gate.sh
...
fail() { echo "TEAMSELECT_GATE FAIL: $*" >&2; exit 1; }
...
if [[ "${CSRETRO_FOREGROUND:-}" == "" && -n "${DISPLAY:-}" ]]; then
	...
	echo "TEAMSELECT_GATE: CSRETRO_FOREGROUND=1 (nativ; Gamescope-Teardown getrennt)"
fi
...
csretro_gate_isolate_begin "teamselect" || fail "isolate"
csretro_gate_isolate_stage_runtime
RUN="${CSRETRO_RUN_DIR}"
LOG="${RUN}/engine.log"
SHOT_DIR="${ROOT}/build/teamselect-shots"
mkdir -p "${SHOT_DIR}"
...
run_one() {
	...
	export CSRETRO_GAMESCOPE_LOG="${RUN}/gamescope-teamselect-${W}x${H}.log"
	...
	wait_log 'CSRETRO_TEAM_GATE_DONE' 90 \
	...
	rg -q 'CSRETRO_TEAM_GATE_FAIL' "${ALL}" && {
		rg 'CSRETRO_TEAM' "${ALL}" | head -20 >&2
	...
	rg -q 'CSRETRO_TEAM_VGUI open .*vip=0' "${ALL}" \
		|| fail "VIP auf ${MAP} darf nicht sichtbar sein ${W}x${H}"
	rg -q 'CSRETRO_TEAM_GATE_ESC visible=0' "${ALL}" \
		|| fail "ESC schließt die Team-Wahl nicht ${W}x${H}"
	rg -q 'CSRETRO_TEAM_CMD jointeam 2' "${ALL}" \
		|| fail "Taste 2 sendet jointeam 2 nicht ${W}x${H}"
	...
	find "${RUN}/cstrike" -maxdepth 2 \( -name '*.tga' -o -name '*.bmp' -o -name '*.png' \) \
		...
		[[ -n "${shot}" ]] || continue
		cp -a "${shot}" "${SHOT_DIR}/teamselect-${W}x${H}.${shot##*.}" 2>/dev/null || true
	done

	echo "TEAMSELECT_GATE PASS ${W}x${H}"
}
...
echo "TEAMSELECT_GATE PASS all resolutions"
