...
1. **Phase 3M** — Mouse/Audio/Video **PASS / Regression** (Brightness/Gamma manueller Recheck offen — `docs/PHASE3M-VIDEO.md`). Keyboard **AUTOMATED PASS / MANUAL RECHECK OPEN**. Adaptive Layout **AUTOMATED PASS / MANUAL ACCEPTANCE OPEN**. Desktop-UI-Optik (Options, Hauptmenü, Create Game, LAN-Browser) ist seit **2026-09-04 visuell abgenommen**. Konsole: **AUTOMATED PASS / MANUAL VISUAL CHECK OPEN**. **Team-Wahl: AUTOMATED PASS + Inhaber visuell bestätigt 2026-09-07 — nicht mehr anfassen, solange der Inhaber nicht ausdrücklich etwas ändert.** Referenz: vollständiger Blur, echte Studio-Figuren mit AK-47 bzw. schallgedämpfter M4A1 über den nativen Player-`weaponmodel`-Pfad vor CS-Retro-Emblemen, Titel/Counts oben, echte Namen und Teamzahlen mittig, Icon-Footer unten rechts. Das sichtbare T- und CT-Modell wechselt bei jedem Öffnen zwischen den vier vorhandenen Klassen, die Teamwaffe bleibt fest. Der Roster kommt laufend aus Scoreboard/Spectator, umfasst alle 32 Slots und der Gate prüft zusätzlich Umbenennung plus Teamwechsel (2/4 statt 3/3). Die Bühne ist auf großen Viewports auf 1440 Pixel Breite begrenzt und zentriert. **Class-Wahl: Inhaber visuell bestätigt 2026-09-07** — vier animierte Originalmodelle in einer gemeinsamen Studioszene, direkte `joinclass 1..4`-Flächen, Auto/Cancel; keine statischen `gfx/vgui`-Portraits. **Buy: Referenzumbau vorhanden, manuelle visuelle Abnahme weiterhin offen.** `TEMP_EXTRA/cstrike15_src` und `TEMP_EXTRA/hl2_src` wurden tatsächlich für Zustands-/Lifecycle-Verträge ausgewertet; die sichtbare Scaleform-Komposition fehlt dort als editierbares Layout, daher folgt die native VGUI2-Komposition dem Referenzbild. Die Hauptseite liegt auf einer zentrierten, begrenzt skalierenden 1280×720-Bühne: nummerierte Kategorien/Produkte, einzeln gerahmte Karten, Blur und Footer. Geld und Buy-Time kommen aus dem Client-HUD. Die rechte, zufällige Player-MDL-Figur verwendet einen Buy-eigenen Yaw/Gait-Zustand statt der Bewegung des lokalen Netzwerkspielers und behält das native `p_ak47`-/`p_m4a1`-Bone-Merge; alle vier T-Modelle wurden einzeln erzwungen geprüft. Karten rendern ausschließlich `w_*.mdl`; deren echte Geometrie wird im Studio-Renderer gold moduliert, ohne TGA, Sprite oder GlowShell. Kurze Produktnamen und getrennte Text-/MDL-Zonen beseitigen die stärksten Überlagerungen; auch 800×600 behält Raster und Figur gleichzeitig. Der Buy-Gate öffnet keine Pistolen-Unterseite mehr, sondern kauft Glock direkt aus dem gemeinsamen Raster. `cstrike/gfx/vgui` bleibt entfernt. Gates: 800×600, 1280×720, 1920×1080 und Live-Resize 1024×768→1280×720; Team/Class-Regression grün. Kein CS2-Katalog. Radio: AUTOMATED PASS + alle drei getrennten Menüs im Spiel bestätigt. Pause ist abgenommen. Spectator/Scoreboard besitzen abgenommene erste Funktionsscheiben, ihr 1:1-Feinschliff bleibt offen. PlayerList bleibt Stub (`menu_playerlist`). **Kein Phase-3-Tag. FOV/3D gesperrt.**
