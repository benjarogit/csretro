#!/usr/bin/env bash
# Options Audio Gate: Localization + Apply/OK/Cancel/Reset + Screenshots.
# Usage: ./scripts/build-menu.sh && ./scripts/vgui-options-audio-gate.sh
...
RUN="${CSRETRO_RUN_DIR:-${ROOT}/build/run-gate/audio}"
...
LOG="${RUN}/engine.log"
SHOT_DIR="${ROOT}/build/options-audio-shots"
MAP="${CSRETRO_SMOKE_MAP:-de_dust}"
cd "${ROOT}"

fail() { echo "OPTIONS_AUDIO_GATE FAIL: $*" >&2; exit 1; }
...
mkdir -p "${RUN}/cstrike/dlls" "${RUN}/cstrike/cl_dlls" "${RUN}/valve" "${RUN}/cstrike/resource" \
	"${GAMEDATA}/cstrike/resource" "${SHOT_DIR}"
...
cp -a "${ROOT}/data/ui-overrides/cstrike/resource/OptionsSubAudio.res" \
	"${GAMEDATA}/cstrike/resource/OptionsSubAudio.res"
cp -a "${ROOT}/data/ui-overrides/cstrike/resource/OptionsSubAudio.res" \
	"${RUN}/cstrike/resource/OptionsSubAudio.res"
# valve/resource fallback for LoadControlSettings search order
mkdir -p "${GAMEDATA}/valve/resource" "${RUN}/valve/resource"
cp -a "${ROOT}/data/ui-overrides/cstrike/resource/OptionsSubAudio.res" \
	"${GAMEDATA}/valve/resource/OptionsSubAudio.res"
...
MP3Volume 0.5
room_hires 2
echo CSRETRO_OPTIONS_AUDIO_GATE_CFG
EOF
...
run_one() {
	...
	export CSRETRO_GAMESCOPE_LOG="${RUN}/gamescope-options-audio-gate-${W}x${H}.log"
	local shot="${SHOT_DIR}/csretro-options-audio-${W}x${H}.png"
	...
	wait_log 'CSRETRO_AUDIO_GATE_DONE' 60 || { stop_engine "${XASH_PID}"; fail "GATE_DONE ${W}x${H}"; }
	wait_log 'CSRETRO_AUDIO_GATE_SHOT_TAKEN|CSRETRO_AUDIO_GATE_SHOT_READY' 30 || true
	sleep 1.5

	if rg -q 'CSRETRO_AUDIO_GATE_FAIL' "${LOG}" 2>/dev/null || rg -q 'CSRETRO_AUDIO_GATE_FAIL' "${CSRETRO_GAMESCOPE_LOG}" 2>/dev/null; then
		rg 'CSRETRO_AUDIO_GATE_FAIL|CSRETRO_LOC_' "${LOG}" "${CSRETRO_GAMESCOPE_LOG}" 2>/dev/null | head -40 >&2 || true
...
echo "OPTIONS_AUDIO_GATE PASS all resolutions"
echo "Shots: ${SHOT_DIR}"
rg -n 'CSRETRO_LOC_|CSRETRO_AUDIO_CVAR|CSRETRO_AUDIO_GATE_OK|room_hires|MP3Volume' \
	"${RUN}"/gamescope-options-audio-gate-*.log 2>/dev/null | head -80 || true
