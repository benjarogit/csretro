...
- **Granaten-Wurf / Live-Teamwechsel / Buy-NVG (2026-09-10):** Alle fünf Granaten teilen denselben Hold-to-Throw-Layer (M1 voll, M2 Underhand, beide mittel; Pin in Freeze, Wurf danach; Switch während Halten bricht ab). Team/Class-Previews nutzen Dummy-Studio-State und Slots ≠ 1, damit ein Live-CT-Körper die T-Vorschau nicht verdreht. Class-Overlay bleibt nach `chooseteam`-Kill über dem Spectator-HUD mit `KEY_DEST_MENU`. Buy-Overview und `BuyModel` überspringen `nvgs`/`nightvision`. Inferno-Werte unverändert. `docs/INFERNO.md`.
