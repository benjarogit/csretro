...
void Border::Paint(int x, int y, int wide, int tall, int breakSide, int breakStart, int breakEnd)
{
    ...
    for (int i = 0; i < _sides[SIDE_LEFT].count; i++)
    {
        ...
        g_pSurfaceNext->DrawSetColor(line->col[0], line->col[1], line->col[2], line->col[3]);
        ...
        if (breakSide == SIDE_LEFT)
        {
            ...
            if (breakStart > 0)
            {
                ...
                g_pSurfaceNext->DrawFilledRect(x + i, y + start_offset,
            ...
            if (breakEnd < (tall - end_offset))
            {
                ...
                g_pSurfaceNext->DrawFilledRect(x + i, y + breakEnd + kBorderThickness,
    ...
    for (int i = 0; i < _sides[SIDE_TOP].count; i++)
    {
        ...
        if (breakSide == SIDE_TOP)
        {
            ...
            {
                // draw before the break Start
                g_pSurfaceNext->DrawFilledRect(x + start_offset, y + i,
            ...
            {
                // draw after break end
                g_pSurfaceNext->DrawFilledRect(x + breakEnd + kBorderThickness, y + i,
    ...
    for (int i = 0; i < _sides[SIDE_RIGHT].count; i++)
    {
        ...
        g_pSurfaceNext->DrawFilledRect(wide - i - kBorderThickness, y + start_offset,
    ...
    for (int i = 0; i < _sides[SIDE_BOTTOM].count; i++)
    {
        ...
        g_pSurfaceNext->DrawFilledRect(x + start_offset, tall - i - kBorderThickness,
...
void Border::PaintCorner(Corner corner, int x, int y, int texture, int texture_half)
{
    ...
    if (_sides[side1].count && _sides[side2].count)
    {
        ...
        g_pSurfaceNext->DrawSetColor(line1->col[0], line1->col[1], line1->col[2], line1->col[3]);
        DrawTexture(texture, x, y, corner);

        if (line1->col != line2->col && texture_half != -1)
        {
            g_pSurfaceNext->DrawSetColor(line2->col[0], line2->col[1], line2->col[2], line2->col[3]);
...
void Border::GetAdjustedLineOffset(const line_t* line, sides_e side, int& start_offset_out, int& end_offset_out)
{
    ...
    if (m_eBackgroundType == BACKGROUND_ROUNDEDCORNERS && _cornerBorderTexturesIsValid)
    {
        ...
        {
            g_pSurfaceNext->DrawGetTextureSize(side == SIDE_TOP ? _cornerBorderTexture1 : _cornerBorderTexture4, w, h);
            start_offset_out += w;

            g_pSurfaceNext->DrawGetTextureSize(side == SIDE_TOP ? _cornerBorderTexture2 : _cornerBorderTexture3, w, h);
            end_offset_out += w;
        }
        else
        {
            g_pSurfaceNext->DrawGetTextureSize(side == SIDE_LEFT ? _cornerBorderTexture1 : _cornerBorderTexture2, w, h);
            start_offset_out += h;

            g_pSurfaceNext->DrawGetTextureSize(side == SIDE_LEFT ? _cornerBorderTexture4 : _cornerBorderTexture3, w, h);
            end_offset_out += h;
        }
    }
}
...
void Border::DrawTexture(int texture, int x, int y, Corner pivot)
{
    ...
    int w, h;
    g_pSurfaceNext->DrawGetTextureSize(texture, w, h);
    ...
    g_pSurfaceNext->DrawSetTexture(texture);
    g_pSurfaceNext->DrawTexturedRect(x, y, x + w, y + h);
}
