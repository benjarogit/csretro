...
**Visuelles Ziel:** Feeling CS-1.6/VGUI2, Dialog-Chrome wie CS:Source (`docs/MENUS.md`). Standard: **eine** gerundete Hülle, innen 90°. Combo ohne Dauer-Fill, Listen-Padding, LAN-Empty-Text einmal bleiben. **Abnahme 2026-09-04:** Der Inhaber ist mit der aktuellen Optik von Hauptmenü, Options, Create Game und LAN-Server-Browser vollständig zufrieden; dieser Stand ist die aktuelle visuelle Produktbaseline. Team-Wahl ist echte VGUI2 (`ClientScheme`, Overlay, kein Frame-Chrome); ihr Referenzumbau ist automatisiert geprüft, die **manuelle visuelle Abnahme ist offen**. Class-Wahl **AUTOMATED + Inhaber-Check Namen ok**; `#Cstrike_Class_Info` nicht mehr roh. Buy **AUTOMATED PASS** (Hauptseite + Pistolen/`glock`); restliche Waffen-/Equipment-Seiten gleicher Codepfad, manueller Check offen.
...
5. **Team/Class/Buy/Radio:** Funktional automatisiert, visueller **1:1-Umbau aktiv**. Team-Wahl: CS:GO-Aufbau (große Figuren vor den Kreisen, Titel oben, echte Namen und Teamzahlen mittig); T/CT wechseln pro Öffnen zufällig zwischen ihren vier Originalmodellen, Waffen bleiben `p_ak47` / `p_m4a1` über den nativen Player-`weaponmodel`-Pfad. Der gemeinsame Scoreboard-/Spectator-Roster umfasst alle 32 Slots; der Gate prüft Umbenennung und Teamwechsel samt neuen Counts. Die Teambühne wächst nur bis 1440 Pixel Breite und bleibt auf größeren/ultrabreiten Viewports zentriert. Class zeigt die vier Originalklassen gleichzeitig in einem Studio-Pass; die vier überlagerten Karten behalten `joinclass 1..4`, Auto und Cancel. Buy: responsive Fünf-Spalten-Übersicht in einer maximal 1440×810 großen Arbeitsfläche; unter 900 Pixel Arbeitsbreite nutzt das Raster die volle Breite und blendet die große Figur/Detailfläche aus. Waffenbilder behalten ihr jeweiliges Quellseitenverhältnis. `radio1`/`radio2`/`radio3` = drei getrennte Listen. **Radio im echten Spiel durch den Inhaber bestätigt (2026-09-05).** Gates: `./scripts/vgui-teamselect-gate.sh`, `./scripts/vgui-classselect-gate.sh`, `./scripts/vgui-buy-gate.sh`, `./scripts/vgui-radio-gate.sh`.
...
**Status: VISUAL ACCEPTED 2026-09-04** für Hauptmenü, Options, Create Game und LAN-Server-Browser. Team-Funktion sowie Class-Namen sind bestätigt; die neue Team-Referenzoptik und der weitere Feinschliff von Class/Buy/Spectator/Scoreboard warten auf manuelle Abnahme. Die neue Konsole braucht ebenfalls ihren manuellen visuellen Check. Neue Desktop-Dialoge müssen die abgenommene Baseline übernehmen; In-Game-Familien folgen ihren eigenen Referenzen.
