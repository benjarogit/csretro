...
**Nächster Schritt:** Spiel vollständig neu starten (Client + Menü + Engine). Waffenkarten: goldene **Seiten**-Silhouetten aus unverzerrten `w_*.mdl` (AK mit Magazin/Schaft, AWP mit Lauf/Zielfernrohr). Granaten stehen als Zylinder; CS-1.6-Flash/HE/Smoke sind dasselbe Mesh, Vest und Vest+Helm dieselbe Datei, Nightvision ohne Icon. Kacheln: Nummer links, Name oben, Silhouette in der Mitte, Preis unten rechts. Figur rechts nicht anfassen. Danach visuell gegen die CS:GO-Referenz prüfen.
