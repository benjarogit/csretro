#!/usr/bin/env bash
# PX4A: Offscreen World+Sprite+Non-Player-Studio. Sichtbarer Frame bleibt Xash.
# Usage: ./scripts/build-client.sh && ./scripts/px4a-offscreen-probe.sh
set -euo pipefail
...
fail() { echo "PX4A_PROBE FAIL: $*" >&2; exit 1; }
...
csretro_gate_isolate_begin "px4a" || fail "isolate"
...
export CSRETRO_GAMESCOPE_LOG="${RUN}/gamescope-px4a.log"
...
wait_log 'offscreen studio proof' 25 || true
...
rg -q 'offscreen studio proof .*differ=1' "${ALL}" || fail "kein Studio-Pixel-Nachweis (sprites_crc == full_crc)"
if ! rg -q 'Studio classified:.*attempted: [1-9].*drawn: [1-9]' "${ALL}"; then
	fail "kein Non-Player Studio attempted>0 drawn>0"
fi
if rg -qi 'STUDIO_EVENTS|events=1' "${ALL}"; then
	fail "STUDIO_EVENTS darf offscreen nicht an sein"
fi

echo "PX4A_PROBE PASS"
echo "PX4A_PROBE log=${ALL}"
rg -n 'offscreen studio|Studio classified:|probe_seq|visible frame stays Xash' "${ALL}" | head -n 40
if [[ -f "${RUN}/cstrike/csretro_offscreen.ppm" ]]; then
	echo "PX4A_PROBE ppm=${RUN}/cstrike/csretro_offscreen.ppm"
fi
exit 0
