...
**Nächster Schritt:** Spiel vollständig neu starten (Client + Menü + Engine). Waffenkarten: Blick von oben auf die flachen `w_*.mdl` (XY-Silhouette), nicht Kante. Figur rechts = eigene Klasse, Yaw wie Class/Team rechts (202 T / 206 CT). Danach visuell gegen die CS:GO-Referenz prüfen.
