#!/usr/bin/env bash
# Options Mouse Gate: Localization + Apply/OK/Cancel/Reset + Screenshots.
# Usage: ./scripts/build-menu.sh && ./scripts/vgui-options-mouse-gate.sh
...
LOG="${RUN}/engine.log"
SHOT_DIR="${ROOT}/build/options-mouse-shots"
MAP="${CSRETRO_SMOKE_MAP:-de_dust}"
cd "${ROOT}"

fail() { echo "OPTIONS_MOUSE_GATE FAIL: $*" >&2; exit 1; }
...
look_filter 0
m_rawinput 1
echo CSRETRO_OPTIONS_MOUSE_GATE_CFG
EOF
...
export CSRETRO_OPTIONS_GATE=1
unset CSRETRO_V1POC CSRETRO_OPTIONS_AUTO 2>/dev/null || true
...
run_one() {
	...
	local shot="${SHOT_DIR}/csretro-options-mouse-${W}x${H}.png"
	...
	wait_log 'CSRETRO_MOUSE_GATE_DONE' 60 || { stop_engine "${XASH_PID}"; fail "GATE_DONE ${W}x${H}"; }
	wait_log 'CSRETRO_MOUSE_GATE_SHOT_TAKEN|CSRETRO_MOUSE_GATE_SHOT_READY' 30 || true
	sleep 1.5

	if rg -q 'CSRETRO_MOUSE_GATE_FAIL' "${LOG}" 2>/dev/null || rg -q 'CSRETRO_MOUSE_GATE_FAIL' "${CSRETRO_GAMESCOPE_LOG}" 2>/dev/null; then
		rg 'CSRETRO_MOUSE_GATE_FAIL|CSRETRO_LOC_' "${LOG}" "${CSRETRO_GAMESCOPE_LOG}" 2>/dev/null | head -40 >&2 || true
	...
	stop_engine "${XASH_PID}"
	echo "OPTIONS_MOUSE_GATE PASS ${W}x${H} shot=${shot}"
}
...
echo "OPTIONS_MOUSE_GATE PASS all resolutions"
echo "Shots: ${SHOT_DIR}"
rg -n 'CSRETRO_LOC_|CSRETRO_MOUSE_CVAR|CSRETRO_MOUSE_GATE_OK|look_filter|m_filter' \
