#!/usr/bin/env bash
# Pause-Gate: In-Game Escape öffnet dasselbe GameUI (Resume/Disconnect), Welt bleibt,
# Titelseiten-Wallpaper nicht. ESC schließt. PlayerList bleibt Stub.
# Usage: ./scripts/build-menu.sh && ./scripts/build-client.sh && ./scripts/vgui-pause-gate.sh
...
fail() { echo "PAUSE_GATE FAIL: $*" >&2; exit 1; }
...
if [[ "${CSRETRO_FOREGROUND:-}" == "" && -n "${DISPLAY:-}" ]]; then
	...
	echo "PAUSE_GATE: CSRETRO_FOREGROUND=1 (nativ; Gamescope-Teardown getrennt)"
fi
...
csretro_gate_isolate_begin "pause" || fail "isolate"
csretro_gate_isolate_stage_runtime
RUN="${CSRETRO_RUN_DIR}"
LOG="${RUN}/engine.log"
SHOT_DIR="${ROOT}/build/pause-shots"
mkdir -p "${SHOT_DIR}"
...
unset CSRETRO_V1POC CSRETRO_OPTIONS_AUTO CSRETRO_OPTIONS_GATE CSRETRO_MAINMENU_GATE \
	...
	CSRETRO_BUY_GATE CSRETRO_RADIO_GATE CSRETRO_SPEC_GATE CSRETRO_SCORE_GATE 2>/dev/null || true
...
run_one() {
	...
	export CSRETRO_GAMESCOPE_LOG="${RUN}/gamescope-pause-${W}x${H}.log"
	...
	wait_log 'CSRETRO_PAUSE_GATE_DONE' 180 \
	...
	rg -q 'CSRETRO_PAUSE_GATE_FAIL' "${ALL}" && {
		rg 'CSRETRO_PAUSE' "${ALL}" | head -40 >&2
		fail "Gate meldet Fehler ${W}x${H}"
	}

	rg -q 'CSRETRO_PAUSE_VGUI open wallpaper=0 blur=1' "${ALL}" \
		|| fail "Pause-VGUI nicht geöffnet oder Wallpaper nicht aus ${W}x${H}"
	rg -q 'CSRETRO_PAUSE_BG blur=1 wallpaper=0' "${ALL}" \
		|| fail "Pause-Blur fehlt ${W}x${H}"
	rg -q 'CSRETRO_PAUSE_GATE_OPEN resume=1 disconnect=1 wallpaper=0 blur=1 raw=0' "${ALL}" \
		|| fail "Resume/Disconnect fehlen oder Roh-Token ${W}x${H}"
	rg -q 'CSRETRO_PAUSE_TITLE text="Pausiert" visible=1 raw=0' "${ALL}" \
		|| fail "Titel Pausiert fehlt oder Roh-Token ${W}x${H}"
	rg -q 'CSRETRO_PAUSE_GATE_ESC visible=0' "${ALL}" \
		|| fail "ESC schließt die Pause nicht ${W}x${H}"
	rg -q 'CSRETRO_PAUSE_VGUI close' "${ALL}" \
		|| fail "Pause-Close fehlt ${W}x${H}"

	mkdir -p "${SHOT_DIR}"
	find "${RUN}/cstrike" -maxdepth 2 \( -name '*.tga' -o -name '*.bmp' -o -name '*.png' \) \
		-printf '%T@ %p\n' 2>/dev/null | sort -n | tail -1 | while read -r _ shot; do
		[[ -n "${shot}" ]] || continue
		cp -a "${shot}" "${SHOT_DIR}/pause-${W}x${H}.${shot##*.}" 2>/dev/null || true
	done

	echo "PAUSE_GATE PASS ${W}x${H}"
}
...
echo "PAUSE_GATE PASS all resolutions"
