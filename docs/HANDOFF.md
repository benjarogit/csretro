...
## Stand 2026-09-20 — #7 Client-Triangles draw-only (implemented / verification pending, Issue OPEN)
...
#7 Client-Triangles Split: `362f1dc` / v0.1.26. Offscreen `191651b` / v0.1.27. Docs dieser Stand.
...
Probes: `./scripts/px3b-offscreen-probe.sh`, `./scripts/px3c-offscreen-probe.sh`, `./scripts/px4a-offscreen-probe.sh`, `./scripts/px4b1-offscreen-probe.sh`, `./scripts/px7-brush-offscreen-probe.sh`, `./scripts/px7-efx-xash-gate.sh`, `./scripts/px7-efx-offscreen-probe.sh`, `./scripts/px7-tri-xash-gate.sh`, `./scripts/px7-tri-offscreen-probe.sh`.
...
**#7 Client-Triangles (implemented / verification pending 2026-09-20)**
...
- Xash-only Gate PASS. Offscreen: 150 Wetterpartikel `mutate=0`, CRC differ=1 pass=trans, Xash `advanced=1`. Spectator-Overview **verification pending**.
...
**Offen vor return 1**
- [#7](https://github.com/benjarogit/csretro/issues/7): Client-Triangles implemented / verification pending (Spectator-Overview ungetestet); `SPR_ANGLED` / Frame-Lerp / Sprite-Lightmap und Brush-Sonderflächen (turb/decals/dlights) DEFERRED. Engine-EFX draw-only ist VERIFIED, Issue bleibt OPEN
...
**Nächster Schritt:** restliche Sprite-Modi / Brush-Sonderflächen (#7). Spectator-Overview manuell, wenn Stock-CS Map-Overview reproduzierbar ist. Kein Viewmodel. `return 1` weiter gesperrt. Kein nächster Slice ohne neue Freigabe.
