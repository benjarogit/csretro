...
{
	const char *functionName = NAME_FOR_FUNCTION((unsigned long)*data);
				...
				for (int j = 0; j < pTest->fieldSize; j++)
				{
					...
					case FIELD_CHARACTER:
						*((char *)pOutputData) = *(char *)pInputData;
						break;
					case FIELD_POINTER:
						*((uintptr_t *)pOutputData) = (uintptr_t)(*(int *)pInputData);
						break;
					case FIELD_FUNCTION:
					{
						if (!Q_strlen((char *)pInputData))
							*((uintptr_t *)pOutputData) = 0;
						else
							*((uintptr_t *)pOutputData) = (uintptr_t)FUNCTION_FROM_NAME((char *)pInputData);
