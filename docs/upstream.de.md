...
- `refs/primext/` ist der nicht gebaute Vergleichsbaum (gitignored). Pin PX1: Tag `continious`, SHA `46fb05b41e58ed887718649e1720313baaac9a35` (2026-08-23). Kein Submodule-/vcpkg-Fetch. Nicht `latest`.
- Erste produktive Integration PX2: nur Render-API-Brücke (`17bd79f`, visuell verifiziert, Issue #4 geschlossen). PX3B: gezielte World/Offscreen-Adaption in `client/render/` (`bbe418d`, visuell zertifiziert, Issue #5 geschlossen, `GL_RenderFrame` bleibt 0). Siehe `docs/research/px1-primext.md`.
...
Der frühere Beschluss „PrimeXT nur beobachten“ ist **ersetzt**. PhysX aus PrimeXT wird für PX2/PX3 nicht übernommen.
