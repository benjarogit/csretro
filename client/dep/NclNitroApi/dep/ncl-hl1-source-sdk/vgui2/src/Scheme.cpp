...
public:
    ...
    [[nodiscard]] VPANEL GetSizingPanel() const { return m_SizingPanel; }
...
// Purpose: Implementation of global scheme interface
//-----------------------------------------------------------------------------
class CSchemeManager : public ISchemeManagerNext
{
public:
    CSchemeManager();
    ~CSchemeManager() override;
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CSchemeManager, ISchemeManager, VGUI_SCHEME_INTERFACE_VERSION_GS, g_Scheme);
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CSchemeManager, ISchemeManagerNext, VGUI_SCHEME_NEXT_INTERFACE_VERSION, g_Scheme);
...
void CScheme::LoadFonts()
{
    ...
    for (KeyValues* kv = m_pData->FindKey("CustomFontFiles", true)->GetFirstSubKey(); kv != NULL; kv = kv->GetNextKey())
    {
        ...
        {
            g_pSurfaceNext->AddCustomFontFile(fontFile);
        }
    }
    ...
    for (KeyValues* kv = m_pData->FindKey("Fonts", true)->GetFirstSubKey(); kv != NULL; kv = kv->GetNextKey())
    {
        for (int i = 0; i < 2; i++)
        {
            ...
            HFont font = g_pSurfaceNext->CreateFont();
...
void CScheme::ReloadFontGlyphs()
{
    ...
    {
        g_pSurfaceNext->GetScreenSize(m_nScreenWide, m_nScreenTall);
    }
    ...
    for (int i = 0; i < m_FontAliases.Count(); i++)
    {
        ...
        for (KeyValues* fontdata = kv->GetFirstSubKey(); fontdata != NULL; fontdata = fontdata->GetNextKey())
        {
            ...
            if (fontdata->GetInt("antialias") && g_pSurfaceNext->SupportsFeature(ISurface::ANTIALIASED_FONTS))
            {
                flags |= ISurface::FONTFLAG_ANTIALIAS;
            }
            if (fontdata->GetInt("dropshadow") && g_pSurfaceNext->SupportsFeature(ISurface::DROPSHADOW_FONTS))
            {
                flags |= ISurface::FONTFLAG_DROPSHADOW;
            }
            if (fontdata->GetInt("outline") && g_pSurfaceNext->SupportsFeature(ISurface::OUTLINE_FONTS))
            {
...
{
    int proH, proW;
    g_pSurfaceNext->GetProportionalBase(proW, proH);
...
{
    int wide, tall;
    g_pSurfaceNext->GetScreenSize(wide, tall);
...
HScheme CSchemeManager::LoadSchemeFromFilePath(const char* fileName, const char* pathID, const char* tag)
{
    ...
    // Load compiled file
    HScheme scheme = LoadSchemeFromFile(fileNameComp, tag);
