...
| **Team / Class / Buy** | Wahl- und Kauf-Viewport | Steam-`.res` bleibt Funktion. Team und Class **visuell bestätigt 2026-09-07**. Buy: CS:GO-Aufbau, CS-1.6-Katalog, `w_*.mdl` als goldene Seiten-Silhouette (Menü-Euler aus Idle-Box, Class-Kamera, ohne Engine-Flatten), Kachel Nummer/Name/Silhouette/Preis, Figur rechts = **eigene Klasse** (`SetPreview` unangetastet) — **visuelle Abnahme offen** | Radio-Karte, GameUI-Frame, Orange-ClientScheme, CS2-Waffenkatalog, Steam-Teamkarten, statische Class-Portraits, Vollbild-Leere, Tactical Shield |
