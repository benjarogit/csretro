...
- **Buy-Karten (Inhaber 2026-09-08):** Gelber Chrom-Rand kam von Studio-Chrome auf `w_*.mdl` plus dunklem Menülicht, nicht von fehlender Geometrie. `EF_CSRETRO_ITEM` lässt die echte Skin, schaltet Chrome aus und setzt Fullbright — kein Weiß-Bind, kein Gold-`rendercolor`. Die Kamera ist dieselbe Blickrichtung wie Class/Team (entlang +X); das Entity liegt mit Pitch 90, Lauf waagerecht. Modellabhängige Roll-/Framing-Korrekturen gleichen die unterschiedlich authorierten CS-1.6-Weltmodelle aus. Die Figur rechts folgt dem tatsächlichen Playerinfo/Userinfo-Modell; `joinclass` ist nur der Übergangs-Fallback. Nightvision bleibt ohne erfundenes Icon. Manuelle Abnahme Buy offen.
