...
- **Buy-Menü (Inhaber 2026-09-07 abgelehnt, Nachzug):** Desktop-Hauptmenü und alte Steam-Kategorieflächen bleiben während In-Game-Menüs aus. Kaufkarten rendern ausschließlich die vorhandenen `w_*.mdl`; ihre erhöhte Kamera wird aus den Engine-Modellgrenzen gerahmt, damit lange Waffen nicht überlaufen und Pistolen nicht als Kante erscheinen. Haupt- und Unterseiten zeigen rechts ein zufälliges animiertes Player-`mdl` mit `p_ak47` beziehungsweise `p_m4a1`, nicht mehr ein TGA oder ein riesiges Weltmodell. Ein echter Gate-Wechsel 1280×720→1024×768 belegt, dass bereits offene Vollbild-Overlays neu zentriert werden; Start-Gates bleiben bei 800×600, 1920×1080 und 2560×1080 grün. `cstrike/gfx/vgui` wird vollständig nicht mehr importiert und aus bestehenden Game-Data-Bäumen entfernt. Kein CS2-Katalog. Manuelle Abnahme des Kaufmenüs bleibt offen.
