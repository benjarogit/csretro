...
void AlertMessage(ALERT_TYPE atype, char* szFmt, ...)
{
	...
	_vsnprintf(buf, sizeof(buf), szFmt, argptr);
	va_end(argptr);
	buf[sizeof(buf) - 1] = '\0';
#else
	va_start(argptr, szFmt);
	vsnprintf(buf, sizeof(buf), szFmt, argptr);
	va_end(argptr);
#endif
...
void CBasePlayerWeapon::FireRemaining(int &shotsFired, float &shootTime, BOOL isGlock18)
{
	...
	if (isGlock18)
	{
		vecDir = m_pPlayer->FireBullets3(vecSrc, Vector(gpGlobals->v_forward), 0.05, 8192, 1, BULLET_PLAYER_9MM, 18, 0.9, m_pPlayer->pev, TRUE, m_pPlayer->random_seed);
	...
	{
		vecDir = m_pPlayer->FireBullets3(vecSrc, Vector(gpGlobals->v_forward), m_fBurstSpread, 8192, 2, BULLET_PLAYER_556MM, 30, 0.96, m_pPlayer->pev, TRUE, m_pPlayer->random_seed);
...
void HUD_WeaponsPostThink( local_state_s *from, local_state_s *to, usercmd_t *cmd, double time, unsigned int random_seed )
{
	...
	// Velaron/cs16-client adf131d: MAX_AMMO_TYPES is one past the last valid index.
