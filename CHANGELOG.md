...
- **Buy-Kacheln und Waffen-Silhouetten (Inhaber 2026-09-08, Engine-Flatten verworfen):** Das Idle-Box-Flattening (dünnste Achse ×0.06, Cull aus) hat Waffen und Granaten zu goldenen Klecksen gemacht. Die Engine färbt nur noch gold; Pitch/Yaw/Roll kommen wieder aus dem Menü (Pfannkuchen waagerecht zur Class-Kamera, Granaten stehend, Ursprung auf die gedrehte Idle-Mitte). Kachel: Nummer links, Name oben in Weiß, Silhouette in der Restfläche, Preis unten rechts. Flash/HE/Smoke und Vest/Vest+Helm sind in CS 1.6 dieselben Weltmeshes; Nightvision bleibt ohne Icon. Playermodel-Pfad (`SetPreview`) unangetastet. Manuelle Abnahme Buy offen.
...
- **Buy-Menü (Inhaber 2026-09-07 abgelehnt, Referenzumbau aktiv):** `TEMP_EXTRA/cstrike15_src` und `TEMP_EXTRA/hl2_src` wurden für Zustands-/Lifecycle-Verträge gelesen; die sichtbare CS:GO-Scaleform-Komposition liegt dort nicht als editierbares Layout vor und wird deshalb nativ nach dem Referenzbild rekonstruiert. Die Hauptseite verwendet eine zentrierte 1280×720-Bühne mit begrenzter Skalierung, nummerierten Kategorien/Produkten, einzeln gerahmten Karten, Blur, Footer und großer rechter MDL-Figur. Geld und Buy-Time kommen aus dem echten Client-HUD. Die Figur rechts ist die eigene Klasse (HUD-Modell), gleicher `SetPreview`-Pfad wie Team/Class (Yaw 206, Sequenz 80 T / 33 CT, natives `p_ak47`-/`p_m4a1`-Bone-Merge). Kaufkarten rendern weiterhin ausschließlich `w_*.mdl`; ein menüspezifischer Studio-Farbpfad macht deren echte Geometrie als goldene Silhouette lesbar, ohne TGA, Sprite oder GlowShell. Kurze Produktnamen und getrennte Text-/Modellzonen verhindern Quetschen. Auf kleinen Viewports bleiben Raster und Figur proportional nebeneinander sichtbar. Der visuelle Gate öffnet keine alte Pistolen-Unterseite mehr, sondern prüft den direkten Glock-Kauf im gemeinsamen Raster. Tests: 800×600, 1280×720 und 1920×1080 sowie Live-Resize 1024×768→1280×720; Team/Class-Regression grün. `cstrike/gfx/vgui` bleibt amputiert. Kein CS2-Katalog und kein importierter Source-/Scaleform-Code. Manuelle visuelle Abnahme bleibt offen.
