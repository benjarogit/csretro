...
wait_log 'CSRetro-Menu:' 20 || fail "kein Desktop-ShowMenu (Team?)"
sleep 1
...
check "no_dialog" "CSRetro-Menu:" || failed=1
check "team_menu" 'CSRetro-Menu: #Team_Select|CSRetro-Menu: #IG_Team_Select|CSRetro-Menu: #Team_Select_Spect' || failed=1
check "manual_team" 'joined team' || failed=1
check "class_or_spawn" 'CSRetro-Menu: #CT_Select|CSRetro-Menu: #Terrorist_Select|game_playerspawn' || failed=1
check "buy" 'CSRETRO_MENU_BUYCMD|CSRetro-Menu: #Buy|CSRetro-Menu: #T_Buy|CSRetro-Menu: #CT_Buy' || failed=1
check "radio" 'CSRETRO_MENU_RADIO1|CSRetro-Menu: #RadioA' || failed=1
...
if [[ "${failed}" -ne 0 ]]; then
    ...
    rg -n 'CSRetro-Menu|CSRETRO_MENU_|joined team|touch/|MenuFactory|shutdown' "${LOG}" >&2 || true
    tail -n 30 "${LOG}" >&2
