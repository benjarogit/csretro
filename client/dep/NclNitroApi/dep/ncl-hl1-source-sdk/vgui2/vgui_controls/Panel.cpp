...
static bool ShouldHandleInputMessage( VPANEL p )
{
	...
	return input()->IsChildOfModalPanel(p);
}
...
void Panel::InternalMousePressed(int code)
{
	...
	if ( ( vgui2::input()->IsKeyDown(KEY_LCONTROL) || vgui2::input()->IsKeyDown(KEY_RCONTROL) ) 
		&& ( vgui2::input()->IsKeyDown(KEY_LALT) || vgui2::input()->IsKeyDown(KEY_RALT) ) )
	{
...
void Panel::OnKeyCodePressed(KeyCode code)
{
	...
	case KEY_UP:
		if ( ( !vgui_nav_lock.IsValid() || vgui_nav_lock.GetInt() == 0 ) && NavigateUp() )
		{
			...
			vgui2::surface()->PlaySound( "UI/menu_focus.wav" );
			handled = true;
		}
		break;
...
void Panel::ApplySettings(KeyValues *inResourceData)
{
	...
	if ( IsX360() || panel_test_title_safe.GetBool() )
	{
		...
		if( bUsesTitleSafeArea )
		{
			...
			int splitX = 0;
			int splitY = 0;
			vgui2::surface()->OffsetAbsPos( splitX, splitY );
...
class CProportionalIntWithScreenspacePropertyX : public vgui2::IPanelAnimationPropertyConverter
{
public:
...
public:
	virtual void GetData( Panel *panel, KeyValues *kv, PanelAnimationMapEntry *entry )
	{
		...
		if ( currentId != -1 &&
			DrawGetTextureFile( currentId, texturename, sizeof( texturename ) ) )
		{
	...
	virtual void SetData( Panel *panel, KeyValues *kv, PanelAnimationMapEntry *entry )
	{
		...
		{
			currentId = DrawGetTextureId( texturename );
			if ( currentId == -1 )
			{
...
private:
	...
	{
		// Remove old filename
		auto it = m_sIdToName.find(id);
		if (it != m_sIdToName.end())
		{
			auto it2 = m_sNameToId.find(it->second);
			if (it2 != m_sNameToId.end())
				it2->second = -1;
		}
		...
		// Set new file name
		m_sIdToName[id] = filename;
		m_sNameToId[filename] = id;
	}

	int DrawGetTextureId(const char *texturename)
	{
		auto it = m_sNameToId.find(texturename);
		if (it == m_sNameToId.end())
			return -1;
		else
			return it->second;
	}
	...
	bool DrawGetTextureFile(int id, char *filename, int maxlen)
	{
		...
		if (it == m_sIdToName.end())
		{
			snprintf(filename, maxlen, "%s", it->second.c_str());
			return true;
		}
		else
		{
	...
	static std::map<std::string, int> m_sNameToId;
	static std::map<int, std::string> m_sIdToName;
};

std::map<std::string, int> CTextureIdProperty::m_sNameToId;
std::map<int, std::string> CTextureIdProperty::m_sIdToName;
...
vgui2::Panel* Panel::GetNavUpPanel()
{
	return m_NavUp;
}

vgui2::Panel* Panel::GetNavDownPanel()
{
	return m_NavDown;
}

vgui2::Panel* Panel::GetNavLeftPanel()
{
	return m_NavLeft;
}

vgui2::Panel* Panel::GetNavRightPanel()
{
	return m_NavRight;
}

vgui2::Panel* Panel::GetNavToRelayPanel()
{
	return m_NavToRelay;
}

vgui2::Panel* Panel::GetNavActivatePanel()
{
	return m_NavActivate;
}

vgui2::Panel* Panel::GetNavBackPanel()
{
	return m_NavBack;
}
...
{
public:
	CPanelMessageMapDictionary() : m_PanelMessageMapPool( sizeof(PanelMessageMap), 32, CUtlMemoryPool::GROW_FAST, "CPanelMessageMapDictionary::m_PanelMessageMapPool", static_cast<int>(alignof(PanelMessageMap)) )
	{
...
{
public:
	CPanelKeyBindingMapDictionary() : m_PanelKeyBindingMapPool( sizeof(PanelKeyBindingMap), 32, CUtlMemoryPool::GROW_FAST, "CPanelKeyBindingMapDictionary::m_PanelKeyBindingMapPool", static_cast<int>(alignof(PanelKeyBindingMap)) )
	{
		m_MessageMaps.RemoveAll();
	}
