...
5. **Team/Class/Buy/Radio:** Funktional automatisiert, visueller Referenzumbau weiter aktiv; frühere Vermerke einer visuellen Abnahme gelten nicht mehr. Team und Class behalten `jointeam`/`joinclass`, Buy verwendet eine gemeinsame Fünf-Spalten-Übersicht mit echten `.res`-Commands, Bildern und Preisen sowie weiterhin erreichbaren klassischen Unterseiten. `radio1`, `radio2` und `radio3` öffnen drei getrennte kompakte Menüs unten links; kein Sammelmenü. Gates: `./scripts/vgui-teamselect-gate.sh`, `./scripts/vgui-classselect-gate.sh`, `./scripts/vgui-buy-gate.sh`, `./scripts/vgui-radio-gate.sh`.
