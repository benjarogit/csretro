...
**Visuelles Ziel:** Feeling CS-1.6/VGUI2, Dialog-Chrome wie CS:Source (`docs/MENUS.md`). Standard: **eine** gerundete Hülle, innen 90°. Combo ohne Dauer-Fill, Listen-Padding, LAN-Empty-Text einmal bleiben. **Abnahme 2026-09-04:** Der Inhaber ist mit der aktuellen Optik von Hauptmenü, Options, Create Game und LAN-Server-Browser vollständig zufrieden; dieser Stand ist die aktuelle visuelle Produktbaseline. **Team-Wahl visuell bestätigt 2026-09-07 — nicht anfassen.** Class-Wahl **visuell bestätigt 2026-09-07**. Buy **AUTOMATED PASS, visuelle Abnahme offen**; Waffenkarten = fullbright `w_*.mdl` mit Skin (Chrome aus, keine Gold-Silhouette), Seitenprofil entlang +X, Figur = `joinclass` (HUD nur wenn Remember leer). Class-T links nach rechts: Phoenix / Elite Crew / Arctic / Guerrilla.
...
5. **Team/Class/Buy/Radio:** Funktional automatisiert. **Team und Class visuell bestätigt 2026-09-07 — nicht anfassen.** Buy-Look **visuelle Abnahme offen**: CS:GO-Komposition mit CS-1.6-Katalog. Zentrierte, nach oben begrenzte 1280×720-Referenzbühne mit gemessenen Spalten, gerundeten Karten, echter HUD-Buy-Time und Geld. Rechts steht die tatsächliche Playerinfo/Userinfo-Klasse; der gemerkte `joinclass`-Button überbrückt nur den Übergang. T: `p_ak47`; CT: `p_m4a1`. Karten zeichnen echte fullbright `w_*.mdl` mit Skin (Chrome aus, kein TGA, keine Gold-Silhouette). Modellabhängige Roll-/Framing-Korrekturen gleichen die verschieden authorierten Weltmodelle aus. Kleine Viewports nutzen die Breite stärker, ab 1280 wächst die Bühne nicht weiter. ESC synchronisiert den Server-Menüzustand; Reopen und Direktkauf öffnen keine alte Pistolen-Unterseite. Gates decken T/Leet, CT/GSG9, 800×600, 1280×720, 1920×1080 und Live-Resize ab. Nightvision hat kein `w_*.mdl`; `cstrike/gfx/vgui` bleibt amputiert. `TEMP_EXTRA/cstrike15_src`/`hl2_src` dient nur dem Lifecycle-Abgleich; kein Source-/Scaleform-Import und kein CS2-Katalog. Team-Wahl: vier zufällige Originalmodelle je Team, `p_ak47`/`p_m4a1`. Class: vier Originalklassen, `joinclass 1..4`. Radio: drei getrennte Listen, im Spiel bestätigt. Gates: `./scripts/vgui-teamselect-gate.sh`, `./scripts/vgui-classselect-gate.sh`, `./scripts/vgui-buy-gate.sh`, `./scripts/vgui-radio-gate.sh`.
...
**Status: AUTOMATED PASS / MANUAL VISUAL CHECK OPEN (Inhaber 11:03: HUD/Complete/Restart FAIL, Fix liegt).** `toggleconsole` ist ein echtes frei belegbares Binding. Die VGUI2-Konsole besitzt Engine-Scrollback, farbige Ausgabe, TextEntry, Befehlsausführung, Up/Down-History, **TAB-Complete** (Prefix, Cmd+CVar), Escape/Frei-Bind zum Schließen, Move/Resize und Geometry-Persistenz. In-Game ist sie ein Overlay: `UI_IsVisible` bleibt false, damit Welt und HUD weiterlaufen (`ui_renderworld` default 0 würde sonst `V_RenderView` überspringen). `sv_restart` / `ResetHUD` schließen die Konsole. Gate: `./scripts/vgui-console-gate.sh`. Manuelle Abnahme der Suggestions und des HUD bei offener Konsole offen.
...
| Konsole | **AUTOMATED PASS / MANUAL VISUAL CHECK OPEN** — Overlay (Welt bleibt), TAB-Complete, schließt bei Restart; `vgui-console-gate.sh`. Inhaber 11:03 FAIL, Fix liegt |
