...
1. **Phase 3M** — Mouse/Audio/Video **PASS / Regression** (Brightness/Gamma manueller Recheck offen — `docs/PHASE3M-VIDEO.md`). Keyboard **AUTOMATED PASS / MANUAL RECHECK OPEN**. Adaptive Layout **AUTOMATED PASS / MANUAL ACCEPTANCE OPEN**. Desktop-UI-Optik (Options, Hauptmenü, Create Game, LAN-Browser) ist seit **2026-09-04 visuell abgenommen**; **Create Server / Options / Find Servers** brauchen eine kurze Nachkontrolle der weniger transparenten `ChromeGlass`-Frames (Alpha 214/200) über dem Titelseiten-Logo. Konsole: **AUTOMATED PASS / MANUAL VISUAL CHECK OPEN**. **Team-Wahl: AUTOMATED PASS + Inhaber visuell bestätigt 2026-09-07 — Layout, Modelle, Farben nicht anfassen.** Nur die T-Emblem-Stern-Rasterung (Coverage statt Integer-Scanlines) ist neu; Kanten ohne Treppchen manuell prüfen, CT-Seite unverändert. **Class-Wahl: Inhaber visuell bestätigt 2026-09-07.** **Tactical Shield ist amputiert.** **Buy: echte texturierte/fullbright `w_*.mdl`, Chrome aus, kein Gold; kameraabhängige Roll-/Framing-Korrekturen; große rechte Figur aus dem tatsächlichen Playerinfo/Userinfo-Modell, `joinclass` nur Übergangs-Fallback; T mit AK, CT mit schallgedämpfter M4A1. ESC/Reopen und Direktkauf bleiben im gemeinsamen Raster. Manuelle visuelle Abnahme offen.** `TEMP_EXTRA/cstrike15_src`/`hl2_src` wurden für Zustands-/Lifecycle-Verträge ausgewertet; kein Source-/Scaleform-Import. Die zentrierte 1280×720-Bühne wächst auf großen Auflösungen nicht weiter und nutzt kleine 4:3-Viewports breiter. Nightvision bleibt ohne Fake-Icon (`w_nvg` existiert nicht). `cstrike/gfx/vgui` bleibt entfernt. Radio: AUTOMATED PASS + drei getrennte Menüs im Spiel bestätigt. Spectator/Scoreboard: Feinschliff offen. PlayerList bleibt Stub. **Kein Phase-3-Tag. FOV/3D gesperrt.**
...
**Nächster Schritt:** Spiel vollständig neu starten (Client + Menü + Engine) und Buy manuell gegen die Referenz abnehmen. Waffenkarten müssen Holz/Metall zeigen, hell und lesbar von der Seite — kein gelber Rand. Figur = aktuell gewählte Klasse; T mit AK, CT mit schallgedämpfter M4A1. Nightvision bleibt ohne erfundenes Icon.
