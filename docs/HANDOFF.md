...
Details: `ROLLEN.md`, `PLATTFORMEN.md`, `SERVER.md`, `INFERNO.md`, `UPSTREAM.md`, `LIZENZEN.md`, `SCHNITTSTELLEN.md`, `MENUS.md`, `GAMEDATA.md`, `PHASEN.md`, `PHASE3-BODY.md` (nur während Phase 3), `PHASE3M.md`, `PHASE3M-KEYBOARD.md`, `PHASE3M-LAYOUT.md`, `PHASE3M-VIDEO.md`, `PHASE3M-METRICS-DIAGNOSIS.md`, `CHANGELOG.md`. Danksagung: `CREDITS.md`.
...
**Katalog-Ausnahme (2026-09-08):** Native **Molotov** (T, $400) und **Incendiary** (CT, $500) — zwei Extra-Granaten, HE/Flash/Smoke bleiben. IDs 32/33, Werte und Import: `docs/INFERNO.md`. Auswahl wie die anderen: Slot-4-Gruppe + `WeapPickup`; Optionen `weapon_molotov`/`weapon_incgrenade`. ZBot kauft und nutzt sie teamabhängig. **Korrektur 2026-09-09:** Das fehlerhafte Cross-Skeleton-Hand-Retarget ist aus dem normalen Import entfernt; Molotov nutzt wieder das intakte animierte 2008-Zippo-Rig. Inc = Smoke-Hände + echte `p_inc`-Dose auf `Bone02`. Inferno-Zahlen sind CS2-Stand. **Nachzug 2026-09-10:** Gemeinsamer Wurf-Layer für alle fünf Granaten (Hold-to-Throw, drei diskrete Stärken, Pin in Freeze, Wurf erst danach, Cancel vor Commit, Velocity = forward×speed + Spieler). Live-`chooseteam`: Team/Class-Previews unabhängig vom Live-Spieler-Slot; Class-Maus bleibt nach Observer-Kill vor dem Spec-HUD. Buy überspringt `nvgs`/`nightvision` (Steam-Res-Reste). **Listen-Test:** Spiel komplett neu starten → Freeze M1 Pin halten, nach Freeze werfen; M2 Kurzwurf HE **und** Incendiary; M1+M2 Medium; Switch während Halten = Cancel. Als CT `chooseteam` → T: Modelle wie Erstjoin, Klasse per Maus. Economy-MR15 und restliche CS2-Waffen nicht anfassen.
...
1. **Phase 3M** — Mouse/Audio/Video **PASS / Regression** (Brightness/Gamma manueller Recheck offen — `docs/PHASE3M-VIDEO.md`). Keyboard **AUTOMATED PASS / MANUAL RECHECK OPEN**. Adaptive Layout **AUTOMATED PASS / MANUAL ACCEPTANCE OPEN**. Desktop-UI-Optik (Options, Hauptmenü, Create Game, LAN-Browser) ist seit **2026-09-04 visuell abgenommen**; **Create Server / Options / Find Servers** brauchen eine kurze Nachkontrolle der weniger transparenten `ChromeGlass`-Frames (Alpha 214/200) über dem Titelseiten-Logo. Konsole: **AUTOMATED PASS / MANUAL VISUAL CHECK OPEN**. **Team-Wahl: AUTOMATED PASS + Inhaber visuell bestätigt 2026-09-07 — Layout, Modelle, Farben nicht anfassen.** Der echte Live-Wechsel `chooseteam` → anderes Team → Klasse ist seit 2026-09-10 im Class-Gate enthalten: serverseitiges `ResetMenu` schließt beide VGUI-Flächen, der angenommene Wechsel respawnt nach der Klassenwahl, Team/Class-Previews sind vom Live-Spieler-Slot getrennt (Dummy-Studio, Indices ≠ 1) und die Class-Maus bleibt nach dem Observer-Kill vor dem Spec-HUD. Yaws/Seqs der Erstjoin-Abnahme unverändert (`150`/`202` Team, Class-Lineup `158/169/191/202`). Nur die T-Emblem-Stern-Rasterung (Coverage statt Integer-Scanlines) ist neu; Kanten ohne Treppchen manuell prüfen, CT-Seite unverändert. **Class-Wahl: Inhaber visuell bestätigt 2026-09-07.** **Tactical Shield und Nightvision sind amputiert.** **Buy: echte texturierte/fullbright `w_*.mdl`, Chrome aus, kein Gold; modellabhängige Roll-/Framing-Korrekturen (u. a. M3/MAC-10/UMP/P90/Galil/AK/Scout/SG552/AWP/G3SG1 aufrecht, Kevlar hochkant wie Granaten). Figur rechts = `joinclass`/Class-Button, HUD nur Fallback. Beim Teamwechsel wird das Buy-Panel neu erzeugt, damit Katalog und Klassenmodell nicht von der alten Seite hängen bleiben. Spielerzahlen stehen über dem Modell-Viewport. GameUI-Flächen dichter (lesbare Texte). Manuelle visuelle Abnahme offen.** `TEMP_EXTRA/cstrike15_src`/`hl2_src` wurden für Zustands-/Lifecycle-Verträge ausgewertet; kein Source-/Scaleform-Import. Die zentrierte 1280×720-Bühne wächst auf großen Auflösungen nicht weiter und nutzt kleine 4:3-Viewports breiter. `cstrike/gfx/vgui` bleibt entfernt. Radio: AUTOMATED PASS + drei getrennte Menüs im Spiel bestätigt. Spectator/Scoreboard: Feinschliff offen. PlayerList bleibt Stub. **Kein Phase-3-Tag. FOV/3D gesperrt.**
...
**Nächster Schritt:** Spiel vollständig neu starten (`./scripts/play.sh`; Client + GameDLL + Menü + Engine). Freeze: M1 Pin, halten, nach Freeze werfen. M2 Kurzwurf HE und Incendiary. M1+M2 Medium. Switch während Halten = Cancel. `chooseteam` als CT → T: Modelle wie Erstjoin, Klasse per Maus. Buy ohne NVG-Karten/`w_nvgs`. Economy-MR15 und restliche CS2-Waffen nicht anfassen.
