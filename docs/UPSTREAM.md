...
| Ref B | `refs/b-cs16-goldsrc/cl_dll/ui/vgui2/cs_vgui2_client.cpp` (`ResourceForMenu` Buy-`.res`, `ItemInfo`, Aliase) + Steam `cstrike/resource/UI/MainBuyMenu.res` / `Buy*.res` | Nur das Mapping: Buy-`.res` als `EditablePanel` (Overlay wie Team/Class), `_CT`/`_TER` nach Team, `.res`-Command = Unterseite, Waffen-Aliase `glock`/`vest`/… → ReGameDLL `HandleBuyAliasCommands`, Steam-`autobuy`/`rebuy` → `cl_autobuy`/`cl_rebuy`. **Nicht** übernommen: deren Software-Renderer, Frame-Chrome, WizardPanel | 2026-09-04 |
