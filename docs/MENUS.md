...
| **Team / Class / Buy** | Wahl- und Kauf-Viewport | Steam-`.res` bleibt Funktion. Team und Class **visuell bestätigt 2026-09-07**. Buy: CS:GO-Aufbau, CS-1.6-Katalog, echte fullbright `w_*.mdl` von der Seite (Chrome aus, kein Gold-Tint), Figur rechts = **aktuelle Playerinfo/Userinfo-Klasse** (`joinclass` als Übergangs-Fallback) — **visuelle Abnahme offen** | Radio-Karte, GameUI-Frame, Orange-ClientScheme, CS2-Waffenkatalog, Steam-Teamkarten, statische Class-Portraits, Vollbild-Leere, Tactical Shield |
