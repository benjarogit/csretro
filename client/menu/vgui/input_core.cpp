...
//
// Purpose: Platform-neutral VGUI input (from InputWin32 logic, no Win32)
//
// $NoKeywords: $
//===========================================================================//
...
class CInputCore : public IInputInternal
{
public:
    CInputCore();
    ~CInputCore();
...
CInputCore g_InputCore;
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CInputCore, IInput, VGUI_INPUT_INTERFACE_VERSION_GS, g_InputCore); // export IInput to everyone else, not IInputInternal!
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CInputCore, IInputInternal, VGUI_INPUTINTERNAL_INTERFACE_VERSION, g_InputCore); // for use in external surfaces only! (like the engine surface)
...
namespace vgui2
{
    IInputInternal* g_pInput = &g_InputCore;
}
...
CInputCore::~CInputCore() = default;

void CInputCore::SetMouseFocus(VPANEL newMouseFocus)
{
...
void CInputCore::SetMouseCapture(VPANEL panel)
{
...
void CInputCore::GetKeyCodeText(KeyCode code, char* buf, int buflen)
{
    if (buf && buflen > 0)
    {
...
void CInputCore::GetCursorPos(int& x, int& y)
{
    //if( g_pSurfaceNext->HasCursorPosFunctions() )
...
bool CInputCore::WasMousePressed(MouseCode code)
{
    return GetCurrentContext()->_mousePressed[code];
}

bool CInputCore::WasMouseDoublePressed(MouseCode code)
{
    return GetCurrentContext()->_mouseDoublePressed[code];
}

bool CInputCore::IsMouseDown(MouseCode code)
{
    return GetCurrentContext()->_mouseDown[code];
}

void CInputCore::SetCursorOveride(HCursor cursor)
{
    //Nothing
}

HCursor CInputCore::GetCursorOveride()
{
    //Nothing
    return NULL_HANDLE;
}

bool CInputCore::WasMouseReleased(MouseCode code)
{
    return GetCurrentContext()->_mousePressed[code];
}

bool CInputCore::WasKeyPressed(KeyCode code)
{
    return GetCurrentContext()->_keyPressed[code];
}

bool CInputCore::IsKeyDown(KeyCode code)
{
    return GetCurrentContext()->_keyDown[code];
}

bool CInputCore::WasKeyTyped(KeyCode code)
{
    return GetCurrentContext()->_keyTyped[code];
}

bool CInputCore::WasKeyReleased(KeyCode code)
{
    return GetCurrentContext()->_keyReleased[code];
}

VPANEL CInputCore::GetAppModalSurface()
{
    return VPanelToHandle(m_DefaultInputContext._appModalPanel);
}

void CInputCore::SetAppModalSurface(VPANEL panel)
{
    m_DefaultInputContext._appModalPanel = VHandleToPanel(panel);
}

void CInputCore::ReleaseAppModalSurface()
{
    m_DefaultInputContext._appModalPanel = nullptr;
}

void CInputCore::GetCursorPosition(int& x, int& y)
{
    auto pCtx = GetCurrentContext();
...
void CInputCore::UpdateMouseFocus(int x, int y)
{
    auto pCtx = GetCurrentContext();
...
void CInputCore::PanelDeleted(VPANEL panel)
{
...
void CInputCore::InternalCursorMoved(int x, int y)
{
    auto pCtx = GetCurrentContext();
...
void CInputCore::InternalMousePressed(MouseCode code)
{
    auto pCtx = GetCurrentContext();
...
void CInputCore::InternalMouseDoublePressed(MouseCode code)
{
    auto pCtx = GetCurrentContext();
...
void CInputCore::InternalMouseReleased(MouseCode code)
{
    auto pCtx = GetCurrentContext();
...
void CInputCore::InternalMouseWheeled(int delta)
{
    auto pCtx = GetCurrentContext();
...
void CInputCore::InternalKeyCodePressed(KeyCode code)
{
    if (!IsKeyCode(code))
        return;
...
void CInputCore::InternalKeyCodeTyped(KeyCode code)
{
    if (!IsKeyCode(code))
        return;
...
void CInputCore::InternalKeyTyped(wchar_t unichar)
{
...
void CInputCore::InternalKeyCodeReleased(KeyCode code)
{
    if (!IsKeyCode(code))
        return;
...
HInputContext CInputCore::CreateInputContext()
{
...
void CInputCore::DestroyInputContext(HInputContext context)
{
...
void CInputCore::AssociatePanelWithInputContext(HInputContext context, VPANEL pRoot)
{
...
void CInputCore::ActivateInputContext(HInputContext context)
{
    m_hContext = context;
}

VPANEL CInputCore::GetMouseCapture()
{
    return VPanelToHandle(m_DefaultInputContext._mouseCapture);
}

bool CInputCore::IsChildOfModalPanel(VPANEL panel)
{
    if (panel)
    {
...
void CInputCore::ResetInputContext(HInputContext context)
{
    InitInputContext(GetCurrentContext());
}

void CInputCore::InitInputContext(InputContext_t* pContext)
{
...
CInputCore::InputContext_t* CInputCore::GetContext(HInputContext context)
{
...
CInputCore::InputContext_t* CInputCore::GetCurrentContext()
{
    return GetContext(m_hContext);
}

void CInputCore::PanelDeleted(VPanel* pFocus, InputContext_t& context)
{
...
VPanel* CInputCore::CalculateNewKeyFocus()
{
    auto pCtx = GetCurrentContext();
