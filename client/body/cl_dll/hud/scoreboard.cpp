...
static void FillScoreboardHudState( ScoreboardHudState *s, int visible, int numTeams )
{
	memset( s, 0, sizeof( *s ) );
	s->visible = visible;
...
int CHudScoreboard :: Draw( float flTime )
{
	if( !ShouldDrawScoreboard( ))
	{
		if( g_pMenu )
		{
			...
			FillScoreboardHudState( &s, 0, m_iNumTeams );
	...
	if( g_pMenu )
	{
		GetAllPlayersInfo();
		ScoreboardHudState s;
		FillScoreboardHudState( &s, 1, m_iNumTeams );
		g_pMenu->SetScoreboardHud( &s );
		return 1;
	}
