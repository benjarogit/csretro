...
- **Buy-Menü (Inhaber 2026-09-07 abgelehnt, Referenzumbau aktiv):** `TEMP_EXTRA/cstrike15_src` und `TEMP_EXTRA/hl2_src` wurden für Zustands-/Lifecycle-Verträge gelesen; die sichtbare CS:GO-Scaleform-Komposition liegt dort nicht als editierbares Layout vor und wird deshalb nativ nach dem Referenzbild rekonstruiert. Hauptseite und Waffen-Unterseiten verwenden eine gemeinsame zentrierte 1280×720-Bühne mit begrenzter Skalierung, kompaktem Fünfspalten-Raster, Blur, Footer und großer rechter MDL-Figur. Geld und Buy-Time kommen jetzt aus dem echten Client-HUD statt aus Platzhaltern. Kaufkarten rendern ausschließlich vorhandene `w_*.mdl`; die zufällige animierte T-/CT-Figur hält `p_ak47` beziehungsweise `p_m4a1`. Tests: 800×600, 1280×720 und 1920×1080 sowie Live-Resize 1024×768→1280×720; Team/Class-Regression grün. `cstrike/gfx/vgui` bleibt amputiert. Kein CS2-Katalog und kein importierter Source-/Scaleform-Code. Manuelle visuelle Abnahme bleibt offen.
