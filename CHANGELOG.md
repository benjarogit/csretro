...
- **Tactical Shield amputiert (Inhaber 2026-09-08):** Kein Schild-Gameplay mehr. Pistolen, Messer, Granaten, C4, Bots, Buy/Rebuy/Autobuy, Pickup, HUD und Schaden behandeln das Schild nicht. `weapon_shield` auf Karten wird beim Spawn entfernt, ohne zu crashen. `ARMOURY_SHIELD=19` bleibt eine leere Map-ID (kein Rutsch der übrigen Armoury-Nummern). Die drei ReGame-API-VTable-Slots `GiveShield`/`DropShield`/`RemoveShield` sind deprecated No-Ops, damit andere Plugin-Slots nicht verrutschen. Buy-Figur ist die eigene Klasse, gleiche Kamera/Pose wie Team/Class rechts (Yaw 202 T / 206 CT). Waffenkarten: dieselbe `pfnRenderScene`-Kamera wie Class/Team, Blick entlang −Z auf die XY-Silhouette der `w_*.mdl` (Gold über `EF_CSRETRO_ITEM`, ohne Backface-Cull). Desktop-`ChromeGlass` Alpha 214/200. T-Emblem-Stern mit Kantendeckung. Economy-Modernisierung ist ein anderer Schritt. Manuelle Abnahme Buy offen.
