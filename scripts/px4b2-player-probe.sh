#!/usr/bin/env bash
# PX4B.2: Remote player studio isolation (Variante B). Visible frame stays Xash.
...
csretro_gate_isolate_begin "px4b2" || fail "isolate"
...
wait_log 'player_info hash BEFORE=' 20 || true
wait_log 'offscreen player proof' 15 || true
wait_log 'probe_seq player look' 15 || true
...
rg -n 'player_info|offscreen player|local player class|Studio classified:|FOLLOW |probe_seq|visible frame stays Xash' "${ALL}" | head -n 80 || true
exit 0
