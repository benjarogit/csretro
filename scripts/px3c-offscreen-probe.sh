#!/usr/bin/env bash
# PX3C: Offscreen World+Sprite-Probe. Sichtbarer Frame bleibt Xash (GL_RenderFrame = 0).
# Startet auf de_aztec, schaut ein env_glow an, wirft HE, dann dust + vid_setmode.
# Usage: ./scripts/build-client.sh && ./scripts/px3c-offscreen-probe.sh
set -euo pipefail
...
fail() { echo "PX3C_PROBE FAIL: $*" >&2; exit 1; }
...
csretro_gate_isolate_begin "px3c" || fail "isolate"
...
export CSRETRO_GAMESCOPE_LOG="${RUN}/gamescope-px3c.log"
...
wait_log 'offscreen sprite proof' 20 || true
...
rg -q 'offscreen world map=maps/de_aztec' "${ALL}" || fail "de_aztec proof fehlt"
rg -q 'offscreen world map=maps/de_dust' "${ALL}" || fail "de_dust proof fehlt"
if rg -q '(^|[[:space:]])empty=1([[:space:]]|$)' "${ALL}"; then
	fail "Offscreen-Pass war leer"
fi
if rg -qi 'GL_RenderFrame.*return 1|custom path took over' "${ALL}"; then
	fail "GL_RenderFrame darf nicht 1 zurückgeben"
fi
rg -q 'offscreen sprite proof world_crc=.*differ=1' "${ALL}" || fail "kein Sprite-Pixel-Nachweis (world_crc == full_crc)"
if ! rg -q 'TempEnt sprite mirrored: [1-9][0-9]* drawn: [1-9]' "${ALL}"; then
	fail "kein reales TempEnt sprite mirrored>0 drawn>0"
fi

echo "PX3C_PROBE PASS"
echo "PX3C_PROBE log=${ALL}"
rg -n 'sprite dump |offscreen sprite crc|offscreen sprite proof|TempEnt sprite mirrored:|Normal sprite mirrored:|probe_seq' "${ALL}" | head -n 40
if [[ -f "${RUN}/cstrike/csretro_offscreen.ppm" ]]; then
	echo "PX3C_PROBE ppm=${RUN}/cstrike/csretro_offscreen.ppm"
fi
exit 0
