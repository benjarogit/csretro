#!/usr/bin/env bash
# #7 brush visual cert: r_csretro_renderer 1, visible frame stays Xash.
# de_aztec world/trans + cs_assault rotating door *11, then dust + vid_setmode.
# Shots: build/px7-brush-cert-shots/ (not committed).
set -euo pipefail
...
MAP="de_aztec"
OUT="${ROOT}/build/px7-brush-cert-shots"

fail() { echo "PX7_BRUSH_VIS FAIL: $*" >&2; exit 1; }
...
csretro_gate_isolate_begin "px7-brush-vis" || fail "isolate"
...
sv_lan 1
_vgui_menus 1
bind "F5" "map de_aztec"
bind "F6" "map cs_assault"
bind "F4" "map de_dust"
bind "F7" "vid_setmode 1024 768"
bind "F8" "quit"
bind "F9" "chooseteam"
bind "F10" "buy"
bind "F11" "exec px7-door-here.cfg"
bind "F12" "exec px7-door-use.cfg"
bind "ESCAPE" "cancelselect"
echo CSRETRO_PX7_BRUSH_VIS_CFG
EOF
cat > "${RUN}/cstrike/px7-door-here.cfg" <<'EOF'
sv_cheats 1
sv_enttools_enable 1
sv_enttools_maxfire 20
noclip
ent_fire 19 movehere
echo CSRETRO_PX7_DOOR_HERE
EOF
cat > "${RUN}/cstrike/px7-door-use.cfg" <<'EOF'
sv_cheats 1
sv_enttools_enable 1
ent_fire 19 use
echo CSRETRO_PX7_DOOR_USE
EOF
...
export CSRETRO_GAMESCOPE_LOG="${RUN}/gamescope-px7-brush-vis.log"
...
sleep 4
shot 01-aztec-team-t-open
press 1
sleep 2
shot 02-aztec-class-t
press 1
sleep 3
shot 03-aztec-spawn-t-viewmodel-hud
sleep 2
shot 04-aztec-spawn-t-followframe
xdotool keydown --window "${WID}" w >/dev/null 2>&1 || true
sleep 2
xdotool keyup --window "${WID}" w >/dev/null 2>&1 || true
sleep 1
shot 05-aztec-walk-world-brush
press F10
sleep 2
shot 06-aztec-buy-t
press Escape
...
press F6
sleep 7
shot 07-assault-team-open
press 2
sleep 2
shot 08-assault-class-ct
press 1
sleep 3
shot 09-assault-spawn-ct-viewmodel-hud
sleep 1
press F11
sleep 2
shot 10-assault-door11-closed
sleep 1
shot 11-assault-door11-closed-followframe
press F12
sleep 2
shot 12-assault-door11-opening
sleep 2
shot 13-assault-door11-open
sleep 1
shot 14-assault-door11-open-followframe

press F4
sleep 6
shot 15-mapchange-dust
press F7
sleep 2
shot 16-vid-setmode
press F8
...
ALL="${RUN}/merged-px7-brush-vis.log"
...
if ! rg -q 'brush mirrored: [1-9]|offscreen brush draw model=' "${ALL}"; then
	fail "keine Brush-Entities im Log"
fi
rg -q 'CSRETRO_PX7_DOOR_HERE' "${ALL}" || fail "Tür-movehere nicht geloggt"
rg -q 'CSRETRO_PX7_DOOR_USE' "${ALL}" || fail "Tür-use nicht geloggt"
if ! rg -q 'model=\*11 origin=696' "${ALL}"; then
	fail "func_door_rotating *11 (origin ~696 2236 48) nicht gesehen"
fi
if ! rg -q 'brush moving index=19 model=\*11|brush placed index=19 model=\*11' "${ALL}"; then
	fail "Index 19 ist nicht *11 (maxplayers 8 erwartet)"
fi

mkdir -p "${OUT}"
cp -a "${SHOTS}/." "${OUT}/"
echo "PX7_BRUSH_VIS PASS"
echo "PX7_BRUSH_VIS shots=${OUT}"
echo "PX7_BRUSH_VIS log=${ALL}"
ls -1 "${OUT}"
rg -n 'brush |offscreen brush|visible frame stays Xash|GL_RenderFrame|PX7_DOOR' "${ALL}" | head -n 50
exit 0
