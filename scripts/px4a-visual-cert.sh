#!/usr/bin/env bash
# PX4A visual cert: r_csretro_renderer 1 on de_aztec (non-player studio must run).
# Visible frame stays Xash. Shots: build/px4a-cert-shots/ (not committed).
set -euo pipefail
...
MAP="de_aztec"
OUT="${ROOT}/build/px4a-cert-shots"

fail() { echo "PX4A_VIS FAIL: $*" >&2; exit 1; }
...
csretro_gate_isolate_begin "px4a-vis" || fail "isolate"
...
bind "F9" "chooseteam"
bind "F10" "buy"
bind "F1" "bot_add_t; echo CSRETRO_PX4A_BOT_ADD_T"
bind "ESCAPE" "cancelselect"
echo CSRETRO_PX4A_VIS_CFG
EOF
...
export CSRETRO_GAMESCOPE_LOG="${RUN}/gamescope-px4a-vis.log"
...
press F1
sleep 2
shot 05-aztec-spawn-t-bot
# Walk a bit so aztec studio worldmodels can enter the view.
xdotool keydown --window "${WID}" w >/dev/null 2>&1 || true
sleep 3
xdotool keyup --window "${WID}" w >/dev/null 2>&1 || true
sleep 1
shot 06-aztec-walk-studio-world
press F10
sleep 2
shot 07-aztec-buy-t
press Escape
...
press F9
sleep 2
shot 08-aztec-team-ct-open
press 2
sleep 2
shot 09-aztec-class-ct
press 1
sleep 3
shot 10-aztec-spawn-ct-viewmodel-hud
sleep 2
shot 11-aztec-spawn-ct-followframe
press F10
sleep 2
shot 12-aztec-buy-ct
press Escape
sleep 1

press F6
sleep 6
shot 13-mapchange-dust
press F7
sleep 2
shot 14-vid-setmode
press F8
...
if ! rg -q 'offscreen studio|Studio classified:|studio draw model=' "${ALL}"; then
	fail "kein Non-Player-Studio-Hinweis im Log (de_aztec Pass muss laufen)"
fi

mkdir -p "${OUT}"
cp -a "${SHOTS}/." "${OUT}/"
echo "PX4A_VIS PASS"
echo "PX4A_VIS shots=${OUT}"
echo "PX4A_VIS log=${ALL}"
ls -1 "${OUT}"
rg -n 'offscreen studio|Studio classified:|studio draw model=|visible frame stays Xash|GL_RenderFrame' "${ALL}" | head -n 40
exit 0
