	...
	while (1)
	{
		...
		if (!_topLevelFocus)
		{
			...
			if (_mainPanel->GetVParent() && _mainPanel->GetVParent() != surface()->GetEmbeddedPanel())
			{
				...
				{
					bFound = true;
					SetCurrentDefaultButton(static_cast<VPANEL>(0));
					break;
				}
			}
		}
	...
	if (best)
	{
        ...
        if (!CanButtonBeDefault(best->GetVPanel()))
        {
			...
			else
			{
				...
				{
					ivgui()->PostMessage(_mainPanel->GetVParent(), new KeyValues("FindDefaultButton"), 0.0f);
				}
			}
        }
        else
...
{
	VPANEL vpanel = panel ? panel->GetVPanel() : static_cast<VPANEL>(0);
...
void FocusNavGroup::SetCurrentDefaultButton(VPANEL panel, bool sendCurrentDefaultButtonMessage)
{
	...
	{
		ivgui()->PostMessage(_currentDefaultButton, new KeyValues("SetAsCurrentDefaultButton", "state", 0), 0.0f);
	}
	...
	if ( sendCurrentDefaultButtonMessage && _currentDefaultButton.Get() != 0)
	{
		ivgui()->PostMessage(_currentDefaultButton, new KeyValues("SetAsCurrentDefaultButton", "state", 1), 0.0f);
	}
}
