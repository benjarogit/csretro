...
void Bitmap::GetSize(int& wide, int& tall)
{
    ...
    if (0 == _wide && 0 == _tall)
    {
        g_pSurfaceNext->DrawGetTextureSize(_id, _wide, _tall);
    }
    wide = _wide;
    tall = _tall;
}
...
void Bitmap::Paint()
{
    ...
    if (!_id)
    {
        _id = g_pSurfaceNext->CreateNewTextureID();
    }
    ...
    g_pSurfaceNext->DrawSetColor(_color[0], _color[1], _color[2], _color[3]);
    g_pSurfaceNext->DrawSetTexture(_id);

    if (_wide == 0)
    {
        GetSize(_wide, _tall);
    }
    g_pSurfaceNext->DrawTexturedRect(_pos[0], _pos[1], _pos[0] + _wide, _pos[1] + _tall);
}
...
void Bitmap::ForceUpload()
{
    ...
    if (!_id)
    {
        _id = g_pSurfaceNext->CreateNewTextureID(_bProcedural);
    }

    if (!_bProcedural)
    {
        g_pSurfaceNext->DrawSetTextureFile(_id, _filename, _filtered, false);
    }

    _uploaded = true;

    _valid = g_pSurfaceNext->IsTextureIDValid(_id);
}
