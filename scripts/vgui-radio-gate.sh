#!/usr/bin/env bash
# Radio-Gate: In-Game Radio als VGUI2-Overlay (kein Steam-.res).
# Team → T → Class → Spawn, dann radio1/2/3. Prüft Öffnen, ESC, coverme, roger, quit.
# Usage: ./scripts/build-menu.sh && ./scripts/build-client.sh && ./scripts/vgui-radio-gate.sh
...
fail() { echo "RADIO_GATE FAIL: $*" >&2; exit 1; }
...
if [[ "${CSRETRO_FOREGROUND:-}" == "" && -n "${DISPLAY:-}" ]]; then
	...
	echo "RADIO_GATE: CSRETRO_FOREGROUND=1 (nativ; Gamescope-Teardown getrennt)"
fi
...
csretro_gate_isolate_begin "radio" || fail "isolate"
csretro_gate_isolate_stage_runtime
RUN="${CSRETRO_RUN_DIR}"
LOG="${RUN}/engine.log"
SHOT_DIR="${ROOT}/build/radio-shots"
mkdir -p "${SHOT_DIR}"
...
unset CSRETRO_V1POC CSRETRO_OPTIONS_AUTO CSRETRO_OPTIONS_GATE CSRETRO_MAINMENU_GATE \
	...
	CSRETRO_BUY_GATE CSRETRO_PAUSE_GATE CSRETRO_SPEC_GATE CSRETRO_SCORE_GATE 2>/dev/null || true
...
run_one() {
	...
	export CSRETRO_GAMESCOPE_LOG="${RUN}/gamescope-radio-${W}x${H}.log"
	...
	wait_log 'CSRETRO_RADIO_GATE_DONE' 180 \
	...
	rg -q 'CSRETRO_RADIO_GATE_FAIL' "${ALL}" && {
		rg 'CSRETRO_RADIO' "${ALL}" | head -40 >&2
	...
	rg -q 'CSRetro-Menu: #RadioA' "${ALL}" && fail "ShowMenu-Legacy für RadioA ${W}x${H}"

	mkdir -p "${SHOT_DIR}"
	find "${RUN}/cstrike" -maxdepth 2 \( -name '*.tga' -o -name '*.bmp' -o -name '*.png' \) \
		-printf '%T@ %p\n' 2>/dev/null | sort -n | tail -1 | while read -r _ shot; do
		[[ -n "${shot}" ]] || continue
		cp -a "${shot}" "${SHOT_DIR}/radio-${W}x${H}.${shot##*.}" 2>/dev/null || true
	done

	echo "RADIO_GATE PASS ${W}x${H}"
}
...
echo "RADIO_GATE PASS all resolutions"
