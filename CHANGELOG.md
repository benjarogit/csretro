...
- **Buy-Karten (Inhaber 2026-09-08, Gold-Tint verworfen):** Die gelben Silhouetten kamen vom menüinternen `EF_CSRETRO_ITEM`-Farbpfad (weiße Textur + Gold). Der ist aus. Karten rendern die echten `w_*.mdl` mit Holz/Metall-Textur; die Kamera blickt von oben auf die große Fläche (Seitenprofil, Lauf waagerecht). Die Figur rechts folgt `joinclass` (Phoenix/Arctic/…), nicht mehr nur dem Userinfo-`model`. Nightvision ohne Icon. Manuelle Abnahme Buy offen.
